...
\begin{document}
	...
	% TODO: Some of my proofs are restatements of my unpublished ArXiv-only work. Do I cite myself (where I have some small mistakes in my proof) or combine into a larger work
    \begin{abstract}
	    ...
	    We show more general formulas for the value of $d(n)$, including a complete characterization when $n = 2^{a}p^{b} - 1$ for $a, b \geq 0$ and $p$ not a Wieferich prime. 
	\end{abstract}
	...
	\begin{lemma}\label{linearityPhiG}
	...
	\begin{proof}
		Let $N(v)$ be the neighborhood of vertex $v$: $v$ and its immediate neighbors.
		Expanding and taking the cardinalities modulo 2,
		\begin{align*}
			N(v) \cap (V_1 \triangle V_2) &= ((N(v) \cap V_1) \cup (N(v) \cap V_2)) \setminus (N(v) \cap V_1 \cap V_2) \\
			\abs{N(v) \cap (V_1 \triangle V_2)} &= \abs{N(v) \cap V_1} + \abs{N(v) \cap V_2} - 2\abs{N(v) \cap V_1 \cap V_2} \\
			&\equiv \abs{N(v) \cap V_1} + \abs{N(v) \cap V_2} \pmod 2.
		\end{align*}
		Thus, for any $v \in V$,
		\begin{align*}
			v \in \Phi_G(V_1 \triangle V_2) &\iff \abs{N(v) \cap (V_1 \triangle V_2)} \equiv 1 \pmod 2 \\
			&\iff \abs{N(v) \cap V_1} + \abs{N(v) \cap V_2} \equiv 1 \pmod 2 \\
			&\iff (v \in \Phi_{G}(V_1)) \text{ XOR } (v \in \Phi_G(V_2)) \\
			&\iff v \in \Phi_G(V_1) \triangle \Phi_G(V_2),
		\end{align*}
		as desired.
	\end{proof}
	...
	\begin{proof}
		...
		Then by lemma \ref{linearityPhiG},
		\begin{align*}
	...
	% TODO: Section on MCP
	% TODO: Maybe most of the even parity covers section could move to a different paper?
