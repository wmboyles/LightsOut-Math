...
\begin{document}
	...
	\end{corollary}
	\begin{proof}
		Evaluating lemma \ref{fieldTrace} to 1,
		\begin{equation*}
			\Tr[\F_q(a) / \F_q]{1} = \sum_{r=0}^{\left[\F_q(a) : \F_q\right] - 1}{1^{q^r}} = \left[\F_q(a) : \F_q\right],
		\end{equation*}
		as desired.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{equation*}
			\rho_2(p^j) = \rho(p) p^{j-1}.
		\end{equation*}
		So, by lemma \ref{fieldExtensionSize}, if $\alpha \in R_j$,
		\begin{equation*}
			\left[\F_2(\alpha) : \F_2\right] = \rho(p) p^{j-1}.
		\end{equation*}
	...
	\begin{corollary}
		...
		\begin{enumerate}
			...
			\item $d(2^n 8191^n - 1) = 2^{n+2} 1001$
		\end{enumerate}
	\end{corollary}
	...
	% TODO: It seems like we only get non-zero d when p is a Fermat or Mersenne prime.
	...
	\begin{proof}
		...
		\begin{equation*}
			\Tr[\F_2(\alpha) / \F_2]{\alpha+1} = \Tr[\F_2(\alpha) / \F_2]{\alpha} + \Tr[\F_2(\alpha) / \F_2]{1} = \Tr[\F_2(\alpha) / \F_2]{\alpha} + 1,
		\end{equation*}
		which is even.
