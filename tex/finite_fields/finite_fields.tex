...
\begin{document}
	...
	\begin{proof}
		...
		So,
		\begin{equation*}
			F_{n+1}(x) = x^{2^{k}-1}(x+1)^{2^k}{g(x)}^{2^k} \qquad F_{n+1}(x+1) = x^{2^k}(x+1)^{2^{k}-1}{g(x+1)}^{2^k},
		\end{equation*}
		% TODO: An AI review flagged the above 2 equations has having the wrong degree. I think it's confused about F_{b}(x)^{2^k} meaning the same as (F_{b}(x))^{2^k}. I wrote it this way instead of putting the exponent closer to F to avoid confusing this with the notation for repeatedly applying F. If the AI is flagging something else, then I'm not sure how the multiplicities would be different from what Hunziker, Machiavelo, and Park state in their lemma 2.6.
	...
	\begin{corollary}\label{2nPlus1Explicit}
		$n$ as $n+1 = 2^s b$ for $s \geq 0$ and $b$ odd.
		Then
		\begin{equation*}
	...
	\begin{lemma}\label{wieferichEquivalence}
		A prime number $p$ is a Wieferich prime if and only if
		\begin{equation*}
			p^2 \mid 2^{\ord[p]{2}} - 1.
		\end{equation*}
	\end{lemma}
	...
	\begin{theorem}\label{pk-1Equalsp-1}
		If $p$ is not a Wieferich prime, then for all $k \in \N$,
		\begin{equation*}
			d(p^k - 1) = d(p-1).
		\end{equation*}
	\end{theorem}
	...
	\begin{corollary}
		...
		\begin{enumerate}
			...
			\item $d(2^n 31^m - 1) = 2^{n+2} 5$
			\item $d(2^n 127^m - 1) = 2^{n+3} 7$
			\item $d(2^n 257^m - 1) = 2^{n+4} 9$
			\item $d(2^n 683^m - 1) = 2^{n+2} 33$
			\item $d(2^n 8191^m - 1) = 2^{n+2} 1001$
		\end{enumerate}
	\end{corollary}
