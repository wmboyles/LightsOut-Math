...
\begin{document}
    ...
    \begin{abstract}
	    ...
	    We explore this game for an $n \times n$ grid and show several new results.
	\end{abstract}
	...
	\begin{proof}
		...
		\end{equation*}
		Taking the degrees, we obtain $d(n-1)$ divides $d(nk-1)$, as desired.
	\end{proof}
	...
	\begin{lemma}[Ore]\label{OreGcd}
		...
		Then
		\begin{equation*}
			(ad,cd) = (a,c)(b,d)\left(\frac{a}{(a,c)},\frac{d}{(b,d)}\right)\left(\frac{c}{(a,c)},\frac{b}{(b,d)}\right).
		\end{equation*}
	\end{lemma}
	\begin{proof}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \deg{(x,x+1)\left(F_{n+1}(x)^2,F_{n+1}(x+1)^2\right)\left(\frac{x+1}{(x,x+1)},\frac{F_{n+1}(x)^2}{\left(F_{x+1}(x)^2,F_{n+1}(x+1)^2\right)}\right)\left(\frac{x}{(x,x+1)},\frac{F_{n+1}(x+1)^2}{\left(F_{n+1}(x)^2,F_{n+1}(x+1)^2\right)}\right)} \\
		...
		When $n+1$ is not divisible by 3, corollary \ref{Threeven} tells us that $F_{n+1}(x)$ is not devisible by $x+1$, so $\delta_n = 0$.
		When $n+1$ is divisible by 3, let $n+1 = b 2^{n}$ for $b,k \geq 0$ and $b$ odd.
	...
	\begin{proof}
		...
		\begin{equation*}
			H_{m+1}(x) = h_{m+1}(y).
		\end{equation*}
		There are only three possible square-free cubic polynomials in $\F_2[y]$:
		\begin{enumerate}
			\item $C_1(y) = y^3 + 1$
			\item $C_2(y) = y^3 + y + 1$
		...
		$C_2$ and $C_3$ are coprime and each of degree 6 in $x$, so $\deg H_{m+1}(x) = \deg{C_1(x)} + \deg{C_2(x)} = 12$, contradicting our assumption that the degree was 6.
		...
		The ranks of $P(x)$ and $Q(x)$ are 17 and 15 respectively.
		Since both 17 and 15 divide 255, and 255 divides $m+1$, then both $P(x)$ and $Q(x)$ must divide $H_{m+1}(x)$.
		Therefore,
		\begin{equation*}
	...
	\begin{lemma}\label{ordpj2}
		...
		Define $h = \ord[p]{n}$ and $c = v_p(2^h - 1)$, where
	...
	\begin{definition}
		...
		\end{equation*}
	\end{definition}
	This is also sometimes called the ``index" of $"\pm 1$ relative to $a$, written $\operatorname{ind}_{a}(\pm 1)$.
	...
	\begin{proof}
		...
		Thus to satisfy both constraints we must have $H_j = r$.
	...
	\begin{proof}
		...
		By definition of $\alpha$, $\alpha^{2^r} = \zeta^{2^r} + \zeta^{-2^r}$, so $\alpha^{2^r} = \alpha$ is equivalent to $\zeta^{2^2} + \zeta^{-2^r} = \zeta + \zeta^{-1}$.
		
		For any non-zero $a,b \in \F_2$, $a + a^{-1} = b + b^{-1} \iff (a+b)(ab+1) = 0$.
	...
	\end{corollary}
	\begin{proof}
		Immediate from setting $a=1$ in lemma \ref{fieldTrace}.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{equation*}
			\Tr[\F_2(\alpha) / \F_2]{\alpha} = \sum_{r=0}^{\rho_2(M)}{\zeta^{2^r} + \zeta^{-2^r}}.
		\end{equation*}
		By the definition of $\rho_2(M)$, the residues $2^0, -2^0, 2^1, -2^1, \ldots, 2^{\rho_2(M)}, 2^{-\rho_2(M)}$. are all distinct modulo $M$.
	...
	\begin{proof}
		...
		\end{equation*}
		as desired.
		% TODO: Find a better/any reasoning for going from sum to mu
	\end{proof}
