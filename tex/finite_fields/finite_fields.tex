...
\begin{document}
	...
	\begin{proof}
		...
		Then
		\begin{equation*}
			1 + \alpha + (\alpha + 1) = 1 + \zeta + \zeta^{-1} + \zeta^{s} + \zeta^{-1} = 0.
		\end{equation*}
		...
		\end{equation}
		% TODO: Check case m vs M here
		In $P_{s-1}$ the only exponent dividing $M$ is 0, so $P_0(X^M) = 1$.
		But if $f(x^m) \mid P_s(x)$, then $f(x)$ must divide every compoenent $P_a(x)$, including $P_0(x) = 1$.
		This is impossible because $f$ isn't constant.
		Thus we have arrived at a contradiction and must conclude that no common roots of $F_{p^k}(x)$ and $F_{p^k}(x+1)$ are in $R_j$ for $j \geq 2$.
