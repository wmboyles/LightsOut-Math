...
\begin{document}
	...
	We resolve a conjecture first stated by Sutner in 1989 about the relationship between the nullity of \textit{Lights Out} boards of size $n$ and $2n + 1$ in the affirmative \cite{Sutner1989}.
	...
	Now that we have a way to express $f_{2n+1}(x)$ and $f_{4n+3}(x)$ as a product of $f_{n}(x)$ and a power of $x$, we simply need a way to express the GCD of products so we can calculate $d(n)$.
	...
	\begin{proof}
		...
		\begin{align*}
			d(4n+3) &= \deg\left(x^3f^4_n(x), (x+1)^3f^4_n(x+1)\right) \\
				...
				&= 4d(n) + 3\delta_n.
		\end{align*}
		Also, our work previously in this proof,
		\begin{align*}
			d(4n+3) &= d(2(2n+1) + 1) \\
				&= 2 d(2n+1) + \delta_{2n+1} \\
				&= 2 \left(2d(n) + \delta_n\right) + \delta_{2n+1} \\
				&= 4d(n) + 2\delta_n + \delta_{2n+1}.
		\end{align*}
		For these two expressions for $d(4n+3)$ to be equal, we must have $\delta_{2n+1} = \delta_n$.
	\end{proof}
